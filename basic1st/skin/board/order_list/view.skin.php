...
<div id="st-view">
	...
	<div class="page-header">
		<h3 class="title"><?=$board['bo_subject']?> <small>견적서 확인</small></h3>
	...
	<div class="info">
		...
		<div class="desc">
			...
			<?php } else { ?>
			견적의뢰자: <?=ST::get_mb_icon($view['mb_id'], $view['name'])?> <strong><?=$view['wr_name'] ?></strong>님<?php if ($is_ip_view) { echo "&nbsp;($ip)"; } ?>&nbsp;&nbsp;&nbsp;
			<?php } ?>			
    ...
    <div class="info">
		<h4 class="subject">
			...
			<small class="comment">[<?=$view['comment_cnt']; ?>]</small><?php } ?>
			<?php if ($view['icon_new']) { ?><small class="new">new</small><?php } ?>
            &nbsp;&nbsp;&nbsp;
            견적의뢰 고객명 :&nbsp;&nbsp;&nbsp; 				
	      <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">
		    <?=ST::get_mb_icon($view['mb_id'], $view['name'])?>
            <strong>
            <?=$view['wr_name'] ?>
        </strong>님</a>
        &nbsp;&nbsp;&nbsp; 
        견적일자: <span class="sound_only">작성일</span><strong><?=date("Y-m-d H:i:s", strtotime($view['wr_datetime'])) ?></strong>
        </h4>
 ...
 <div>
  ...
  <?php
	...
	<div class="actions buttons">
		<?php ob_start()?>	
		 <div class="left">
			<a href="<?=$list_href ?>" class="btn btn-sm btn-default">견적목록</a>
			<?php if ($prev_href) { ?><a href="<?=$prev_href ?>" class="btn btn-sm btn-default">이전견적</a><?php } ?>
			<?php if ($next_href) { ?><a href="<?=$next_href ?>" class="btn btn-sm btn-default">다음견적</a><?php } ?>
			<?php if ($search_href) { ?><a href="<?=$search_href ?>" class="btn btn-sm btn-default">검색</a><?php } ?>
			<?php if ($copy_href) { ?><a href="<?=$copy_href ?>" class="btn btn-sm btn-danger" onclick="board_move(this.href); return false;">견적복사</a><?php } ?>
			<?php if ($move_href) { ?><a href="<?=$move_href ?>" class="btn btn-sm btn-danger" onclick="board_move(this.href); return false;">견적이동</a><?php } ?>
			<?php if ($delete_href) { ?><a href="<?=$delete_href ?>" class="btn btn-sm btn-danger" onclick="del(this.href); return false;">견적삭제</a><?php } ?>
		 </div>
		<div class="right">
			...
			<?php if ($reply_href) { ?><a href="<?=$reply_href ?>" class="btn btn-sm btn-default">견적답변</a><?php } ?>
			<?php if ($update_href) { ?><a href="<?=$update_href ?>" class="btn btn-sm btn-primary"><i class="fa fa-pencil" aria-hidden="true"></i> 견적수정</a><?php } ?>
			<?php if (!$update_href and $write_href) { ?><a href="<?=$write_href ?>" class="btn btn-sm btn-primary"><i class="fa fa-pencil" aria-hidden="true"></i> 등록하기</a><?php } ?>
		</div>	
		<?php
