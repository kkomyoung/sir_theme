...
<div id="st-basic">
	...
	<div class="actions info">
		...
		<div class="right">
			...
			<?php if ($is_category) { ?>
			<div class="btn-group">
				<button type="button" class="btn btn-info btn-sm dropdown-toggle" data-toggle="dropdown"><?=$sca? $sca: '견적종류선택'?> <span class="caret"></span></button>
				<ul class="dropdown-menu pull-right" role="menu">
					<?=$category_option ?>
				</ul>
			</div>
			<?php } ?>					
			<?php if ($write_href) { ?><a href="<?=$write_href.($sca? '&sca='.$sca: '')?>" class="btn btn-sm btn-primary"><i class="fa fa-pencil" aria-hidden="true"></i> 등록하기</a><?php } ?>	
		</div>
	</div>
	...
	<table class="table table-hover">
		...
		<tr>
			...
			<?php if ($is_checkbox) { ?>
			<th width="50" scope="col" class="chk">
				<label for="chkall" class="sound_only">현재 페이지 게시물 전체</label>
				<input type="checkbox" id="chkall" onclick="if (this.checked) all_checked(true); else all_checked(false);">
			</th>
			<?php } ?>
			<th width="280"scope="col" class="sbj"><span class="견적종류">견적종류</span> / <span class="차종">차종</span></th>
            
			<th width="100" scope="col" class="name hidden-xs-v">고객명</th>
            <th width="108" scope="col">담당자</th>
            <th width="80" scope="col">직함</th>
            <th width="120" scope="col">연락처</th>
            <th width="120" scope="col"><span class="견적상황">견적상황</span></th>
			...
			<th scope="col" class="date"><?=subject_sort_link('wr_datetime', $qstr2, 1) ?>견적일</a></th>
		</tr>
		</thead>
		<tbody>
		...
		<tr onclick="return onRowClick(event, '<?=$list[$i]['href'] ?>');"<?php if ($list[$i]['is_notice']) echo ' class="notice"'?>>
            ...
            <td align="center"><?php  echo substr($list[$i][wr_3],0,strlen($list[$i][wr_3])-3) .'*';  ?></td>
            <td align="center"><?=$list[$i][wr_4]?></td>
            <td align="center"><?php  echo substr($list[$i][wr_5],0,strlen($list[$i][wr_5])-4) .'****';  ?></td>
            
            
            <td align="center"class="td_num" style="width:100px">
			<?php if ($list[$i]['wr_10']=="견적완료") { ?><div style="padding:0px 0 0 0;width:100px;height:22px;background:#FF3061;color:#fff;font-weight:bold;">견적완료</div> 
			<?php }elseif($list[$i]['wr_10']=="견적중"){ ?><div style="padding:0px 0 0 0;width:100px;height:22px;background:#5AD103;color:#fff;font-weight:bold;">견적중</div>
			<?php }elseif($list[$i]['wr_10']=="견적접수"){ ?><div style="padding:0px 0px 0 0;width:100px;height:22px;background:#878ED3;color:#fff;font-weight:bold;">견적접수</div>
			<?php } ?>
            </td>
	...
	<div class="actions button">
		...
		<?php if ($write_href) { ?>
		<div class="right">
			<a href="<?=$write_href.($sca? '&sca='.$sca: '')?>" class="btn btn-sm btn-primary"><i class="fa fa-pencil" aria-hidden="true"></i> 견적등록</a>
		</div>
		<?php } ?>
	</div>
	</form>
	...
	<fieldset id="search" class="search">
		...
		<select name="sfl" id="sfl" class="form-control input-sm">
			<option value="wr_subject"<?=get_selected($sfl, 'wr_subject', true); ?>>차종검색</option>
			<option value="wr_content"<?=get_selected($sfl, 'wr_content'); ?>>기타내용</option>
			...
			<option value="wr_name,1"<?=get_selected($sfl, 'wr_name,1'); ?>>고객명검색</option>
			<option value="wr_10"<?=get_selected($sfl, 'wr_10'); ?>>견적상황별검색</option>
		</select>
